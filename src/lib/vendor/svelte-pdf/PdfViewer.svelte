<script lang="ts">
	...
	const createLinkOverlay = (
		...
		const canvasRect = viewport.convertToViewportRectangle([x1, y1, x2, y2]) as [
			number,
			number,
			number,
			number
		...
		{#if passwordError === true}
			<div class="password-viewer">
				...
				<div class="password-container">
					...
					<button onclick={onPasswordSubmit} class="password-button"> Submit </button>
				</div>
			</div>
